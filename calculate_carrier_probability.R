...
set.seed(3)
carrier_prob_plot<-ggplot(sample_data,aes(x=pi,y=carrier_prob,colour=Carrier_Status))+
...
ggsave(carrier_prob_plot,filename=paste(output,"carrier_probabilities_plot.tiff",sep="/"),dpi=300, width=7.5,height=3.5,units="in")
write.table(sample_data, file=paste(output,"sample_probability_table.txt",sep="/"),sep="\t",quote=FALSE,row.names=FALSE)
